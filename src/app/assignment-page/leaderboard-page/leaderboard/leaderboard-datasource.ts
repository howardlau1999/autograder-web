...
export class LeaderboardDataSource extends DataSource<LeaderboardItem> {
  ...
  constructor(data$: Observable<LeaderboardItem[]>, search$: Observable<string>) {
    super();
    this.data$ = data$.pipe(
      tap((data) => {
        this.data = data;
  ...
  private filterData(data: LeaderboardItem[]): LeaderboardItem[] {
    if (!this.search) return data;
    return data.filter((item) => {
      return (
        item.studentId.includes(this.search) ||
